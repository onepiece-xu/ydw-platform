...
public class MKTAdbClient implements IAdbClient{
	...
	public Boolean disconnect() {
		...
		if (String.format(AdbCommand.STRING_ADB_DISCONNECTED, ip, port).equals(string)) {
			logger.info("{} disconnect success" ,ip);
		}
		logger.info("{} end disconnect" ,ip);
		return false;
	}
...
//		clearAppData(appPackageName);
    ...
    public DevDiskDetailed getDiskSize () {
		...
		if (string == null) {
			...
			string = runShell(String.format(AdbCommand.MKT_STRING_ADB_DF_STORAGE_EMULATED,ip));
			if (string == null)
			return null;
		}
