...
//            ftpClient.enterLocalPassiveMode();
    ...
    public static void downloadSingleFile(FTPClient ftpClient, String absoluteLocalDirectory, String relativeRemotePath) {
        ...
        try {
            ...
            if (ftpFile != null && ftpFile.isFile()) {
                ...
                outputStream.flush();
                outputStream.close();
                logger.info(">>>>>FTP服务器文件下载完毕*********" + ftpFile.getName());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public static void downloadSingleFile(FTPClient ftpClient, String relativeRemotePath,OutputStream outputStream) {
        /**如果 FTP 连接已经关闭，或者连接无效，则直接返回*/
        if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
            logger.error(">>>>>FTP服务器连接已经关闭或者连接无效*********");
            return;
...
//        }
            ...
            if (uploadFile.isDirectory()) {
                ...
                for (int i = 0; i < listFiles.length; i++) {
                    ...
                    if (loopFile.isDirectory()) {
                        ...
                        input.close();
                        logger.info(">>>>>文件上传成功****" + loopFile.getPath());
                    }
                }
            } else {
                /**如果被上传的是文件时*/
     ...
     * @throws Exception
     */
    public static void uploadFile(FTPClient ftpClient, String remoteDir, String fileName, InputStream inputStream) {
        /**如果 FTP 连接已经关闭，或者连接无效，则直接返回*/
        if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
            logger.error(">>>>>FTP服务器连接已经关闭或者连接无效*****放弃文件上传****");
            return;
     ...
     *
     * @param ftpClient  连接成功有效的 FTPClinet
     * @param filePath 待上传的文件 或 文件夹(此时会遍历逐个上传)
     * @throws Exception
     */
    public static void uploadFiles(FTPClient ftpClient, String filePath, InputStream input) {
        /**如果 FTP 连接已经关闭，或者连接无效，则直接返回*/
        if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
            logger.error(">>>>>FTP服务器连接已经关闭或者连接无效*****放弃文件上传****");
            return;
